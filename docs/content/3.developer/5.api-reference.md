...
*   **Method**: `POST`
*   **Description**: Submits various health data points (e.g., weight, steps, active calories, custom measurements) to the system. This endpoint is primarily used for integrations with external health tracking services.
...
*   **Request Body**: An array of health data objects, or a single health data object.
    ...
    *   **Fields**:
        ...
        *   `type` (required, string): The type of health data. Supported types include:
            *   `weight`: Body weight.
            *   `step`: Number of steps.
            *   `Active Calories`: Calories burned through activity.
            *   Any other string: Will be treated as a custom measurement category. If the category does not exist, it will be created.
        *   `date` (required, string): The date of the measurement in `YYYY-MM-DD` format.
        *   `timestamp` (optional, string): The exact timestamp of the measurement in ISO 8601 format. If provided, `entryHour` will be extracted.
    *   **Examples**:
        *   **Weight**:
            ```json
            [
...
*   **Method**: `PUT`
*   **Description**: Updates an existing daily check-in measurement entry.
*   **Authentication**: JWT Token.
*   **Path Parameters**:
    *   `id` (required, string): The ID of the check-in entry to update.
*   **Request Body**: Same as POST, but `entry_date` is required and `id` is used for lookup.
*   **Responses**:
    *   `200 OK`:
        ```json
        {
          "message": "Check-in measurements updated successfully.",
          "data": { "id": "uuid", "user_id": "uuid", "entry_date": "2025-08-19", "weight": 71.0 }
        }
        ```
    *   `400 Bad Request`:
        ```json
