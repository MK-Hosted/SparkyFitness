...
async function getFoodDataProviders(userId) {
  // This function will be removed from foodService later
  try {
    ...
    log(
      "error",
      `Error fetching food data providers for user ${userId} in foodService:`,
      error
    );
    throw error;
  }
...
async function getFoodDataProvidersForUser(authenticatedUserId, targetUserId) {
  ...
  try {
    ...
    log(
      "error",
      `Error fetching food data providers for target user ${targetUserId} by ${authenticatedUserId} in foodService:`,
      error
    );
    throw error;
  }
...
async function createFoodDataProvider(authenticatedUserId, providerData) {
  ...
  try {
    ...
    log(
      "error",
      `Error creating food data provider for user ${authenticatedUserId} in foodService:`,
      error
    );
    throw error;
  }
...
async function updateFoodDataProvider(
  ...
  try {
    ...
    log(
      "error",
      `Error updating food data provider ${providerId} by user ${authenticatedUserId} in foodService:`,
      error
    );
    throw error;
  }
...
async function getFoodDataProviderDetails(authenticatedUserId, providerId) {
  ...
  try {
    ...
    log(
      "error",
      `Error fetching food data provider details for ${providerId} by user ${authenticatedUserId} in foodService:`,
      error
    );
    throw error;
  }
...
async function deleteFoodDataProvider(authenticatedUserId, providerId) {
  // This function will be removed from foodService later
  try {
    const success = await externalProviderService.deleteExternalDataProvider(
      authenticatedUserId,
      providerId
    );
    return success;
    ...
    log(
      "error",
      `Error deleting food data provider ${providerId} by user ${authenticatedUserId} in foodService:`,
      error
    );
    throw error;
  }
    ...
    if (
      ...
      if (defaultVariant) {
        const updatedVariantData = {
          ...
          polyunsaturated_fat:
            foodData.polyunsaturated_fat ?? defaultVariant.polyunsaturated_fat,
          monounsaturated_fat:
            foodData.monounsaturated_fat ?? defaultVariant.monounsaturated_fat,
...
async function getFoodVariantsByFoodId(authenticatedUserId, foodId) {
  ...
  try {
    ...
    if (foodOwnerId !== authenticatedUserId) {
      log(
        "warn",
        `getFoodVariantsByFoodId: Forbidden - User ${authenticatedUserId} does not own food ${foodId}.`
      );
      throw new Error(
...
async function createFoodEntry(authenticatedUserId, entryData) {
  try {
    ...
    log(
      "error",
      `Error creating food entry for user ${authenticatedUserId} in foodService:`,
      error
    );
    throw error;
  }
...
async function addMealFoodsToDiary(
  ...
  try {
    ...
    log(
      "error",
      `Error adding meal foods to diary for user ${authenticatedUserId}, meal ${mealId}:`,
      error
    );
    throw error;
  }
